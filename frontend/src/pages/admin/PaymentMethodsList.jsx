...
export default function PaymentMethodsList() {
  ...
  const fetchCurrencies = async () => {
    try {
      const res = await fetch("https://api.movecash.online/api/currency", {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      setCurrencies(data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = form.id
        ? `https://api.movecash.online/api/payment_method/${form.id}`
        : "https://api.movecash.online/api/payment_method";
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://api.movecash.online/api/payment_method/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
